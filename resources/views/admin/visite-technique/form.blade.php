<div class="form-group {{ $errors->has('engin_name') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="engin_name" type="text" id="engin_name" value="{{ isset($visitetechnique->engin_name) ? $visitetechnique->engin_name : ''}}" >
    
    @foreach($engins as $enginT)
            <option value="{{ $enginT->engin_name }}" @if( isset($visitetechnique) && $enginT->id == $visitetechnique->engin_id) selected
                @endif )>
                {{ $enginT->engin_name }}
            </option>
        @endforeach
...
<div class="form-group {{ $errors->has('date_debut_val') ? 'has-error' : ''}}">
    <label for="date_debut_val" class="control-label">{{ 'Date Debut Validation' }}</label>
...
<div class="form-group {{ $errors->has('date_fin_val') ? 'has-error' : ''}}">
    <label for="date_fin_val" class="control-label">{{ 'Date Fin Validation' }}</label>
...
<div class="form-group {{ $errors->has('engin_id') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="engin_id" type="number" id="engin_id" value="{{ isset($visitetechnique->engin_id) ? $visitetechnique->engin_id : ''}}" required>
  
  
    @foreach($engins as $enginT)
            <option value="{{ $enginT->id }}" @if( isset($visitetechnique) && $enginT->id == $visitetechnique->engin_id) selected
                @endif )>
