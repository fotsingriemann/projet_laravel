<div class="form-group {{ $errors->has('engin_name') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="engin_name" type="text" id="engin_name" value="{{ isset($patente->engin_name) ? $patente->engin_name : ''}}" required>
   
   
    @foreach($engins as $enginT)
            <option value="{{ $enginT->engin_name }}" @if( isset($patente) && $enginT->id == $patente->engin_id) selected
                @endif )>
                {{ $enginT->engin_name }}
            </option>
        @endforeach
...
<div class="form-group {{ $errors->has('engin_id') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="engin_id" type="number" id="engin_id" value="{{ isset($patente->engin_id) ? $patente->engin_id : ''}}" required>
   
   
    @foreach($engins as $enginT)
            <option value="{{ $enginT->id }}" @if( isset($patente) && $enginT->id == $patente->engin_id) selected
                @endif )>
